...
public:
    Delay() : lt6(0), lt12(0), lt18(0), lt24(0), mt24(0) {}

    void checkAndIncrement(double timeDelay)
    {
        if (timeDelay <= 6)
            lt6++;
        else if (timeDelay <= 12)
            lt12++;
        else if (timeDelay <= 12)
            lt18++;
        else if (timeDelay <= 12)
...
int main(int argc, char *argv[])
{
    ...
    {
        for (auto &v : graphDV[d])
            edgesDV.insert(make_pair(d, v));
    }
    for (int w = 0; w < graphWU.size(); w++)
    {
        for (auto &u : graphWU[w])
            edgesWU.insert(make_pair(w, u));
    }
    ...
    for (int j = 0; j < noOfUCEdges; j++)
    {
        ...
        for (auto x : cIndexGenerator.getVal(c))
            curFlights.push_back(x.first);
    ...
    function<bool(int, int)> checkDefaultSolution = [&](int d, int u) -> bool
    {
        for (auto &wd : edgesWD)
        {
            if (d == wd.second)
            {
                for (auto &wu : edgesWU)
                {
                    if (wd.first == wu.first and wu.second == u)
                        return true;
                }
            }
        }
