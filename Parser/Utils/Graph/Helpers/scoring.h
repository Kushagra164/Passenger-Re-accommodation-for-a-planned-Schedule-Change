...
int pnrScore(int journeyId, CLASS_CD proposed)
{
    ...
    for (auto &passenger_id : pnr->passengers)
    {
        ...
        if (passenger->ssrCode != SSR_CD_NULL)
            pnr_score += SSR_SCORE;
...
long long getFlightScoreWithTimeDiff(Time ArrivalTimeDiff, Time DepartureTimeDiff)
{
    ...
    if (ArrivalTimeDiff <= Time(6, 0))
        score += ARRIVAL_DELAY_LT_6_SCORE;
    else if (ArrivalTimeDiff <= Time(12, 0))
        score += ARRIVAL_DELAY_LT_12_SCORE;
    else if (ArrivalTimeDiff <= Time(24, 0))
        score += ARRIVAL_DELAY_LT_24_SCORE;
    else if (ArrivalTimeDiff <= Time(48, 0))
        score += ARRIVAL_DELAY_LT_48_SCORE;

    if (DepartureTimeDiff <= Time(6, 0))
        score += DEPARTURE_DELAY_LT_6_SCORE;
    else if (DepartureTimeDiff <= Time(12, 0))
        score += DEPARTURE_DELAY_LT_12_SCORE;
    else if (DepartureTimeDiff <= Time(24, 0))
        score += DEPARTURE_DELAY_LT_24_SCORE;
    else if (DepartureTimeDiff <= Time(48, 0))
        score += DEPARTURE_DELAY_LT_48_SCORE;
...
long long getFlightScore(int curJourneyID, int nextInventoryID)
{
    ...
    if (journeyMap[curJourneyID]->flights.size() == 1)
    {
        score += scheduleMap[inventoryToScheduleMap[journeyMap[curJourneyID]->flights[0]]]->equipmentNo ==
                         scheduleMap[inventoryToScheduleMap[nextInventoryID]]->equipmentNo
                     ? EQUIPMENT_SCORE
                     : 0;
    }
    return score;
