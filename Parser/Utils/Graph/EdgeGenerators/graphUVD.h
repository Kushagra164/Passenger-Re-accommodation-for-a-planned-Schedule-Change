    ...
    for(int curJourneyID:AffectedJourneys){
        ...
        for(auto [curInventoryID, curInventory]:inventoryMap){
            if((CancelledFlights.find(curInventoryID) == CancelledFlights.end()) 
                    && srcCity == scheduleMap[inventoryToScheduleMap[curInventoryID]]->srcCity
                    && destCity == scheduleMap[inventoryToScheduleMap[curInventoryID]]->destCity
                    && ((getDepTimeDiff(srcInventoryID, curInventoryID)
                            +getArrTimeDiff(destInventoryID, curInventoryID))
                ...
                for(int toClassCD = 0; toClassCD < 4; toClassCD++){
                    ...
                    if(isSufficientInventoryAvailable(curPaxCnt, curInventoryID, static_cast <CLASS_CD> (toClassCD))){
                        ...
                        long long curScore = origScore*getFlightScore(curJourneyID, curInventoryID)
