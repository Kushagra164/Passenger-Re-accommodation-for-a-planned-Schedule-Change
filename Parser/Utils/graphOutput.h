...
using namespace std;
template <typename T>
int calEdges(ExtendableVector<vector<T>> &adj)
{
    int edges = 0;
...
void graphOutput(ofstream &fw)
{
    ...
    for (int v = 0; v < modV; v++)
    {
        ...
        if (p.second == FC)
            avl_inv = inventoryMap[inv_id]->fcTotalInventory - inventoryMap[inv_id]->fcBookedInventory;
        else if (p.second == BC)
            avl_inv = inventoryMap[inv_id]->bcTotalInventory - inventoryMap[inv_id]->bcBookedInventory;
        else if (p.second == PC)
            avl_inv = inventoryMap[inv_id]->pcTotalInventory - inventoryMap[inv_id]->pcBookedInventory;
        else
            avl_inv = inventoryMap[inv_id]->ecTotalInventory - inventoryMap[inv_id]->ecBookedInventory;
